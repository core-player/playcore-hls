...
export function getVeeRVersion() {
  return config.version;
};

export function getVeeRUA() {
  if (localStorage.ua) {
    return localStorage.ua;
  }
  ...
  const veerUA = `Mozilla/5.0 (${os} ${osVersion}; ${browser}; ${version}) ${app}/${getVeeRVersion()}`;

  return veerUA;
};

export function checkSourceType(src) {
...
export function fovToUV(fov){
  const phiLength = (fov.hfov / 360) * Math.PI * 2;
  const thetaLength = (fov.vfov / 180) * Math.PI;
...
export function getFormatViaStereo(stereo) {
  if (stereo === 'left_right') {
    return '3DLR';
  } else if (stereo === 'top_bottom') {
    return '3DTB';
  } else if (stereo === 'bottom_top') {
    return'3DBT';
  }
  return '2D';
};
...
export function i18n(contents, key) {
  const lang = getCookie('veer-lang') || 'en';
  key = key || 'en';
  if (contents[lang]) {
    return contents[lang][key];
  }
  return '';
};
...
export function eulerToArray(euler) {
  if (typeof euler === 'object' && euler.order) {
...
export const isSafari = isMobileJS.apple.device && _isSafari();
